...
function addNotificationWithStudyHubDetails(studyHubDetails) {
    if (studyHubDetails.type === "Mentorship") {
      fetch("../assets/php/notification/process_getInvitation-mentorship.php")
        .then((response) => response.json()) // Assuming the PHP script returns JSON data
        .then((dataToUse) => {
          MMdetail = dataToUse; // Store the JSON data in the variable
  
          // Now you can use MMdetail for further processing or display
          const firstObject = MMdetail[0];
  
          // Accessing individual properties using dot notation
          const user_ID = firstObject.user_ID;
          const name = firstObject.name;
          const profilePic = firstObject.profile_pic;
          console.log(user_ID);
          console.log(name);
          ...
          imgLink.href = `viewProfile.php?user_ID=${user_ID}`; // Set the URL with studyhub_ID parameter
          ...
          col3.innerHTML = `${name} invited you to a Mentorship<br>
          <a href="javascript:void(0)" class="btn btn-success" onclick="approveMM(${studyHubDetails.studyhub_ID}, 'Accepted', '${studyHubDetails.invite_ID}')">Accept</a>
          <a href="javascript:void(0)" class="btn btn-danger" onclick="approveMM(${studyHubDetails.studyhub_ID}, 'Rejected', '${studyHubDetails.invite_ID}')">Reject</a>`;
  ...
  function displayNotifications(invite_ID) {
    fetch("../assets/php/notification/process_getInvitation.php")
      .then((response) => response.json()) // Assuming the PHP script returns JSON data
      .then((dataToUse) => {
        if (invite_ID) {
  ...
  function approval(studyHub_ID, answer, invite_ID) {
    ...
    if (
      ...
      $.ajax({
        url: "../assets/php/notification/process_invitationResponse.php",
        method: "POST",
  ...
  function approveMM(mentor_ID, answer, invite_ID) {
    var answerText = answer.toLowerCase();
    if (answerText === "accept") {
      answerText = answerText.slice(0, -2);
    } else {
    ...
    if (
      ...
      $.ajax({
        url: "assets/php/notification/process_invitationResponse-mentorship.php",
        method: "POST",
